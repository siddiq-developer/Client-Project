...
<body class="home page-template-default page page-id-7 woocommerce-js wpb-js-composer js-comp-ver-5.6 vc_responsive">
	<div id="page">
		...
		<div id="page-title" class="page-title" >
			...
			<div class="container">
				<div class="row">
					<div id="page-title-text" class="page-title-text col-xs-12 col-sm-12 col-md-12 col-lg-12"><h1>{{$event->title}}</h1></div>
					<div id="breadcrumb-text" class="breadcrumb-text col-xs-12 col-sm-12 col-md-12 col-lg-12"><ul class="breadcrumbs"><li><a href="{{route('home')}}">Home</a></li><li>{{$event->title}}</li></ul></div>
						...
						<article id="post-1284" class="pray-single-blog post-1284 events type-events status-publish has-post-thumbnail hentry category-events-upcoming">
							<div class="entry-blog entry-post">
								<div class="entry-header">
									<img width="770" height="537" src="{{\Storage::disk('public')->url('app/public/events/'.$event->image)}}" class="attachment-pray-img-blog size-pray-img-blog wp-post-image" alt="{{$event->title}}" srcset="{{\Storage::disk('public')->url('app/public/events/'.$event->image)}}" sizes="(max-width: 770px) 100vw, 770px"><div class="cms-grid-media  has-thumbnail"></div>				<h2 class="entry-title">{{$event->title}}</h2>
							...
							<div class="share-causes">
								<div class="wrap-share">
									...
									<div class="post-share">
										<a href="http://www.facebook.com/sharer.php?u={{route('event_single',['slug'=>$event->slug])}}/&amp;t={{$event->title}}" title="Share on Facebook."><span class="share-box"><i class="social_facebook"></i></span></a>
										
										<a href="http://twitter.com/home/?status={{$event->title}} - {{route('event_single',['slug'=>$event->slug])}}/" title="Tweet this!"><span class="share-box"><i class="social_twitter"></i></span></a>
